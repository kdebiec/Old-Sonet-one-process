...
#ifdef RS_ENABLE_WALL
#include "services/wall/p3wallservice.h"
#endif

#ifdef RS_ENABLE_VOIP
...
//	std::cerr << "gxs_passwd (core 2) = " << (void*)&rsInitConfig->gxs_passwd<< " \"" << rsInitConfig->gxs_passwd << "\""<< std::endl;
    ...
    RsGeneralDataService* gxswalls_ds = new RsDataService(currGxsDir + "/", "gxswalls_db",
                                                        RS_SERVICE_GXS_TYPE_WALLS, NULL, rsInitConfig->gxs_passwd);

    RsWall::p3WallService *mGxsWalls = new RsWall::p3WallService(gxswalls_ds, NULL, mGxsIdService);

    // create GXS photo service
    RsGxsNetService* gxswalls_ns = new RsGxsNetService(
                    RS_SERVICE_GXS_TYPE_WALLS, gxswalls_ds, nxsMgr,
                    mGxsWalls, mGxsWalls->getServiceInfo(),
        mReputations, mGxsCircles,mGxsIdService,
        pgpAuxUtils);

    mGxsWalls->setNetworkExchangeService(gxswalls_ns) ;
...
#ifdef RS_ENABLE_WALL
        pqih->addService(gxswalls_ns, true);
#endif
...
#ifdef RS_ENABLE_VOIP
    std::cerr << "Starting p3VoIPService" << std::endl;
    p3VOIPService *pVOIP = new p3VOIPService(serviceCtrl);
    pqih -> addService(pVOIP,true);
#endif
...
#ifdef RS_ENABLE_WALL
    mConfigMgr->addConfiguration("gxswalls.cfg", gxswalls_ns);
#endif

#ifdef RS_ENABLE_VOIP
    //mConfigMgr->addConfiguration("voip.cfg", pVOIP);
#endif
...
#ifdef RS_ENABLE_WALL
    RsWall::rsWall = mGxsWalls;
    startServiceThread(mGxsWalls, "gxs walls");
    startServiceThread(gxswalls_ns, "gxs walls ns");
#endif
