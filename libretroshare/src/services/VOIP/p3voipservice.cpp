...
p3VOIPService::p3VOIPService(p3ServiceControl *cs)
     : mVOIPMtx("p3VOIPService"), mServiceControl(cs)
{
...
RsServiceInfo p3VOIPService::getServiceInfo()
{
...
int	p3VOIPService::tick()
{
#ifdef DEBUG_VOIP
     //   std::cerr << "ticking p3VOIPService" << std::endl;
#endif
...
int	p3VOIPService::status()
{
        return 1;
}

int	p3VOIPService::sendPackets()
{
...
void p3VOIPService::sendBandwidthInfo()
{
...
int p3VOIPService::sendVoipHangUpCall(const RsPeerId &peer_id, uint32_t flags)
{
...
int p3VOIPService::sendVoipAcceptCall(const RsPeerId& peer_id, uint32_t flags)
{
        RsVOIPProtocolItem *item = new RsVOIPProtocolItem ;
...
int p3VOIPService::sendVoipRinging(const RsPeerId &peer_id, uint32_t flags)
{
    std::cerr << "p3VOIPService::sendVoipRinging" << std::endl;
    std::cerr << peer_id.toStdString() << std::endl;
        RsVOIPProtocolItem *item = new RsVOIPProtocolItem ;
...
int p3VOIPService::sendVoipBandwidth(const RsPeerId &peer_id,uint32_t bytes_per_sec)
{
        RsVOIPProtocolItem *item = new RsVOIPProtocolItem ;
...
int p3VOIPService::sendVoipData(const RsPeerId& peer_id,const RsVOIPDataChunk& chunk)
{
#ifdef DEBUG_VOIP
        ...
        {
                std::cerr << "(EE) p3VOIPService: cannot send chunk data. Unknown data type = " << chunk.type << std::endl;
                delete item ;
                return false ;
        }
...
void p3VOIPService::sendPingMeasurements()
{
        /* we ping our peers */
        /* who is online? */
    if(!mServiceControl)
...
#ifdef DEBUG_VOIP
        std::cerr << "p3VOIPService::sendPingMeasurements() @ts: " << ts;
        std::cerr << std::endl;
#endif
    ...
    for(it = onlineIds.begin(); it != onlineIds.end(); it++)
        {
#ifdef DEBUG_VOIP
                std::cerr << "p3VOIPService::sendPingMeasurements() Pinging: " << *it;
                std::cerr << std::endl;
#endif
...
#ifdef DEBUG_VOIP
                std::cerr << "p3VOIPService::sendPingMeasurements() With Packet:";
                std::cerr << std::endl;
...
void p3VOIPService::handleProtocol(RsVOIPProtocolItem *item)
{
...
#ifdef DEBUG_VOIP
                        std::cerr << "p3VOIPService::handleProtocol(): Received protocol ring item." << std::endl;
#endif
                break ;

                case RsVOIPProtocolItem::VoipProtocol_Ackn: mNotify->notifyReceivedVoipAccept(item->PeerId(), (uint32_t)item->flags);
#ifdef DEBUG_VOIP
                        std::cerr << "p3VOIPService::handleProtocol(): Received protocol accept call" << std::endl;
#endif
                break ;

                case RsVOIPProtocolItem::VoipProtocol_Close: mNotify->notifyReceivedVoipHangUp(item->PeerId(), (uint32_t)item->flags);
#ifdef DEBUG_VOIP
                        std::cerr << "p3VOIPService::handleProtocol(): Received protocol Close call." << std::endl;
#endif
                break ;
                case RsVOIPProtocolItem::VoipProtocol_Bandwidth: mNotify->notifyReceivedVoipBandwidth(item->PeerId(),(uint32_t)item->flags);
#ifdef DEBUG_VOIP
                        std::cerr << "p3VOIPService::handleProtocol(): Received protocol bandwidth. Value=" << item->flags << std::endl;
#endif
                break ;
                default:
                        std::cerr << "p3VOIPService::handleProtocol(): Received protocol item # " << item->protocol << ": not handled yet ! Sorry" << std::endl;
                break ;
        }

}

void p3VOIPService::handleData(RsVOIPDataItem *item)
{
        RsStackMutex stack(mVOIPMtx); /****** LOCKED MUTEX *******/
...
bool p3VOIPService::getIncomingData(const RsPeerId& peer_id,std::vector<RsVOIPDataChunk>& incoming_data_chunks)
{
        RsStackMutex stack(mVOIPMtx); /****** LOCKED MUTEX *******/
        ...
        for(std::list<RsVOIPDataItem*>::const_iterator it2(it->second.incoming_queue.begin());it2!=it->second.incoming_queue.end();++it2)
        {
                ...
                {
                        std::cerr << "(EE) p3VOIPService::getIncomingData(): error. Cannot handle item with unknown type " << type_flags << std::endl;
                        delete *it2 ;
                        free(chunk.data) ;
                        continue ;
                }
...
bool	p3VOIPService::recvItem(RsItem *item)
{
...
int p3VOIPService::handlePing(RsVOIPPingItem *ping)
{
        /* cast to right type */

#ifdef DEBUG_VOIP
        std::cerr << "p3VOIPService::handlePing() Recvd Packet from: " << ping->PeerId();
        std::cerr << std::endl;
#endif
...
#ifdef DEBUG_VOIP
        std::cerr << "p3VOIPService::handlePing() With Packet:";
        std::cerr << std::endl;
...
int p3VOIPService::handlePong(RsVOIPPongItem *pong)
{
        /* cast to right type */

#ifdef DEBUG_VOIP
        std::cerr << "p3VOIPService::handlePong() Recvd Packet from: " << pong->PeerId();
        std::cerr << std::endl;
        pong->print(std::cerr, 10);
#endif
...
#ifdef DEBUG_VOIP
        std::cerr << "p3VOIPService::handlePong() Timing:";
        std::cerr << std::endl;
...
int	p3VOIPService::storePingAttempt(const RsPeerId& id, double ts, uint32_t seqno)
{
        RsStackMutex stack(mVOIPMtx); /****** LOCKED MUTEX *******/
...
int	p3VOIPService::storePongResult(const RsPeerId &id, uint32_t counter, double ts, double rtt, double offset)
{
        RsStackMutex stack(mVOIPMtx); /****** LOCKED MUTEX *******/
        ...
        {
#ifdef DEBUG_VOIP
                std::cerr << "p3VOIPService::storePongResult() ERROR Severly Delayed Measurements!" << std::endl;
#endif
        }
        else
        {
...
uint32_t p3VOIPService::getPongResults(const RsPeerId& id, int n, std::list<RsVOIPPongResult> &results)
{
        RsStackMutex stack(mVOIPMtx); /****** LOCKED MUTEX *******/
...
VOIPPeerInfo *p3VOIPService::locked_GetPeerInfo(const RsPeerId &id)
{
...
void p3VOIPService::setVoipATransmit(int t)
{
        _atransmit = t ;
        IndicateConfigChanged() ;
}
void p3VOIPService::setVoipVoiceHold(int vh)
{
        _voice_hold = vh ;
        IndicateConfigChanged() ;
}
void p3VOIPService::setVoipfVADmin(int vad)
{
        _vadmin = vad ;
        IndicateConfigChanged() ;
}
void p3VOIPService::setVoipfVADmax(int vad)
{
        _vadmax = vad ;
        IndicateConfigChanged() ;
}
void p3VOIPService::setVoipiNoiseSuppress(int n)
{
        _noise_suppress = n ;
        IndicateConfigChanged() ;
}
void p3VOIPService::setVoipiMinLoudness(int ml)
{
        _min_loudness = ml ;
        IndicateConfigChanged() ;
}
void p3VOIPService::setVoipEchoCancel(bool b)
{
        _echo_cancel = b ;
        IndicateConfigChanged() ;
}

RsTlvKeyValue p3VOIPService::push_int_value(const std::string& key,int value)
{
        RsTlvKeyValue kv ;
        kv.key = key ;
...
int p3VOIPService::pop_int_value(const std::string& s)
{
...
bool p3VOIPService::saveList(bool& cleanup, std::list<RsItem*>& lst)
{
        cleanup = true ;
...
bool p3VOIPService::loadList(std::list<RsItem*>& load)
{
...
RsSerialiser *p3VOIPService::setupSerialiser()
{
